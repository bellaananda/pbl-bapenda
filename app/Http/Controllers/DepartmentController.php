...
class DepartmentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $base_uri = "https://api.klikagenda.com/api";
        try {
            $responseValidate = $client->post("{$base_uri}/validate-department", [
                'headers' => [
                    'Accept' => 'application/json',
                    'Authorization' => 'Bearer ' . $token,
                ],
        ...
        $response = $client->post("{$base_uri}/departments", [
            'headers' => [
                'Accept' => 'application/json',
                'Authorization' => 'Bearer ' . $token,
            ],
    ...
    public function update(Request $request, string $id)
    {
        ...
        $response = $client->post("{$base_uri}/departments/{$id}", [
            'headers' => [
                'Accept' => 'application/json',
                'Authorization' => 'Bearer ' . $token,
            ],
        ...
        if ($response->getStatusCode() == 200 && $data['success']) {
            return redirect('/bidang')->with('success_message', 'Bidang berhasil diubah!');
        } else {
            return back()->with('error_message', 'Bidang gagal diubah!');
        }
    }
    ...
    public function destroy(string $id)
    {
        ...
        if ($response->getStatusCode() == 200 && $data['success']) {
            return redirect('/bidang')->with('success_message', 'Bidang berhasil dihapus!');
        } else {
            return back()->with('error_message', 'Bidang gagal dihapus!');
        }
    }
}
